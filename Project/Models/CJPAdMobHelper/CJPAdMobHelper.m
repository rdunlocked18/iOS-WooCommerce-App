...
- (void)startWithViewController:(UIViewController *)contentController
{
    ...
    //Reload ads when app is resumed
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(createBanner)
...
- (void)createBanner
{
    ...
    //[testDevIDs addObject:@"YourTestDevice"];
    ...
    if (_tagForChildDirectedTreatment!=nil) {
        [GADMobileAds.sharedInstance.requestConfiguration tagForChildDirectedTreatment:_tagForChildDirectedTreatment.boolValue];
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_showingAd) {
        ...
        contentFrame.size.height -= bottomPadding + bannerFrame.size.height;
...
- (void)adViewDidReceiveAd:(GADBannerView *)bannerView
{
    ...
    [UIView animateWithDuration:0.25
                     ...
                     completion:^(BOOL finished){
                         ...
                         if (finished) [self->_containerView bringSubviewToFront:self->_adMobView];
                     }];
}
